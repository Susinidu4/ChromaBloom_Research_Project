// src/pages/Gamified_Knowledge_Builder/Quize/QuizeEdit.jsx
...
export default function QuizeEdit() {
  ...
  const [form, setForm] = useState({
    ...
    correct_answer: 1,
    answers: [], // [{image_no, img_url}]
    correct_img_url: "", // keep old url so we can show it
  });

  // ✅ NEW: selected new correct image file (optional)
  const [correctFile, setCorrectFile] = useState(null);

  // ✅ NEW: selected new answer files (optional; if any selected -> should upload ALL)
  const [answerFiles, setAnswerFiles] = useState([]); // File|null aligned with answers
  ...
  const onSubmit = async (e) => {
    ...
    // ✅ multipart needed if correctFile OR any answer file chosen
    ...
    try {
      ...
      if (useMultipart) {
        // if user selects any answer replacement -> must upload ALL answer images
        if (hasNewAnswerImages) {
          const missing = answerFiles.findIndex((f) => !f);
          if (missing !== -1) {
            setMsg({
              type: "error",
              text: `You selected answer replacement. Please upload ALL answer images (missing Answer #${
                missing + 1
              }).`,
            });
            setSaving(false);
        ...
        const payload = {
          ...
          // keep old url if correctFile not selected
        ...
        // reload to reflect new URLs
        const refreshed = await QuizeService.getById(id);
        const q = refreshed?.data;
        ...
        setForm((p) => ({
          ...p,
          correct_img_url: q?.correct_img_url || p.correct_img_url,
          answers: normalizedAnswers,
        }));
        ...
        const payload = {
          ...
          difficulty_level: form.difficulty_level,
          correct_answer: Number(form.correct_answer),
          correct_img_url: form.correct_img_url, // keep as is
  ...
  const correctPreviewUrl = (() => {
    if (correctFile) return URL.createObjectURL(correctFile);
    if (form.correct_img_url) return form.correct_img_url;
    const idx = Number(form.correct_answer || 1) - 1;
    return form.answers?.[idx]?.img_url || "";
  })();
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField("correct_answer", Number(e.target.value))
                  }
                >
            ...
            <div className="p-4 rounded-2xl bg-[rgba(233,221,204,0.45)] border border-black/5">
              ...
              <div className="mt-3 flex flex-col gap-2">
                <div className="text-[12px] opacity-75 font-semibold">
                  Replace correct image (optional)
                </div>
                ...
                {correctFile ? (
                  <div className="text-[12px] opacity-80">
                    New: <b>{correctFile.name}</b>
                  </div>
                ) : (
                  <div className="text-[12px] opacity-80">
                    No new correct image selected
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[rgba(233,221,204,0.45)] border border-black/5">
                    ...
                    >
                      ...
                      <div className="mt-3 flex flex-col gap-2">
                        ...
                        <input
                          ...
                          accept="image/*"
                          onChange={(e) =>
                            onPickAnswerFile(idx, e.target.files?.[0] || null)
                          }
                          className="px-3 py-2 rounded-xl border border-black/15 bg-white text-[13px]"
                        />
              ...
              <p className="mt-3 mb-0 text-[12px] opacity-75">
                If you upload any new answer image, backend replaces the whole answers set. So upload ALL answer images for a clean update.
              </p>
            </div>
            ...
            <div className="text-[12px] opacity-60">Answers: {answerCount}</div>
          </form>
        )}
      </div>
    </div>
